...
int wmain(int argc, wchar_t* argv[])
{
	...
	if (argc == 2) {
		file = argv[1];
		log = new wchar_t[] {L"D:\\OS\\UPDATE.log"};
	}
	else if (argc == 4) {
		file = argv[1];
		userName = argv[2];
		...
		<< "---->HT_UPDATE {fileName}{username?}{password?}{logfile?}" << std::endl;
	if (file != nullptr && log != nullptr) {
		...
		int but = MessageBox(NULL, L"START?", L"HT_UPDATE", MB_OKCANCEL | MB_ICONINFORMATION);
		if (but == IDCANCEL) {
		...
		while (true) {
			...
			HT::Element* elementInMemory = HTAPI::Get(hthandle, &element);
			if (!elementInMemory) {
				WriteRowInLog(hf, element, FALSE);
				std::cout << key << '\t' << 0 << std::endl;
			}
			else {
				int* newPayload = new int();
				ZeroMemory(newPayload, sizeof(int));
				*newPayload = *((int*)elementInMemory->payload) + 1;
				bool res = HTAPI::Update(hthandle, &element, newPayload, elementInMemory->payloadlength);
