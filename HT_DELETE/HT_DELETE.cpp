...
int wmain(int argc, wchar_t* argv[])
{
	...
	if (argc == 2) {
		file = argv[1];
		log = new wchar_t[] {L"D:\\OS\\DELETE.log"};
	}
	else if (argc == 4) {
		file = argv[1];
		userName = argv[2];
		...
		<< "---->OS_DELETE {fileName}{username?}{password?}{logfile?}" << std::endl;
	if (file != nullptr && log != nullptr) {
		...
		int but = MessageBox(NULL, L"START?", L"HT_DELETE", MB_OKCANCEL | MB_ICONINFORMATION);
		if (but == IDCANCEL) {
		...
		while (true) {
			...
			bool res = HTAPI::Delete(hthandle, &element);
